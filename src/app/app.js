...
async function saveVideos(value) {
...
async function exportAllVideos() {
    ...
    showToast(`${videos.length} ${videos.length === 1 ? "video" : "videos"} exported.`);
}
// ============================================================
// IMPORT
// ============================================================
...
function isValidExportFile(data) {
...
// IMPORT MODAL
// ============================================================
function showImportModal(videoCount) {
    return new Promise((resolve) => {
      ...
      `;
        ...
        const closeButton = backdrop.querySelector(".import-modal-close");
        const cancelButton = backdrop.querySelector(".import-modal-cancel");
        const confirmButton = backdrop.querySelector(".import-modal-confirm");
        let resolved = false;
        const close = (value) => {
            if (resolved) {
                return;
...
// MERGE IMPORT
// ============================================================
async function mergeImportedVideos(importedVideos) {
    ...
    for (const importedVideo of importedVideos) {
         ...
         */
        const existingVideo = mergedVideos.find((video) => video.platform === importedVideo.platform &&
        ...
        for (const importedNote of importedVideo.notes) {
            const noteExists = existingVideo.notes.some((note) => note.id === importedNote.id);
            if (!noteExists) {
        ...
        existingVideo.updatedAt = Math.max(existingVideo.updatedAt, importedVideo.updatedAt);
    }
    await saveVideos(mergedVideos);
    await loadVideos();
    showToast(`Import completed: ${addedVideos} ${addedVideos === 1 ? "video" : "videos"} and ${addedNotes} ${addedNotes === 1 ? "note" : "notes"} added.`);
}
// ============================================================
// IMPORT BUTTON
// ============================================================
...
importFile?.addEventListener("change", async () => {
    ...
    try {
        ...
        showToast(error instanceof Error ? error.message : "Error during import.", "error");
    }
});
// ============================================================
// EXPORT BUTTON
...
document.addEventListener("click", async (event) => {
    ...
    const button = target.closest(".delete-note-button");
    if (!button) {
        return;
    }
    ...
    const video = videos.find((item) => item.platform === platform && item.externalId === externalId);
    const confirmed = await showConfirmModal("Delete this note?", "The note will be permanently deleted.");
    if (!confirmed) {
        return;
    }
    ...
    try {
        ...
        if (video && video.notes.length === 0) {
            const deleteVideo = await showConfirmModal("Delete parent video?", "This was the last note for this content. Do you wish to delete the video as well?");
            if (deleteVideo) {
                await sendDeleteMessage({
                    type: "DELETE_CONTENT",
                ...
                videos = videos.filter((item) => !(item.platform === video.platform &&
...
document.addEventListener("click", async (event) => {
    const target = event.target;
    const button = target.closest(".delete-video-button");
    if (!button) {
        return;
    }
    const platform = button.dataset.platform;
    ...
    const confirmed = await showConfirmModal("Delete this content?", `“${title}” and all related notes will be deleted.`);
    if (!confirmed) {
        return;
    }
    button.disabled = true;
    try {
        ...
        videos = videos.filter((item) => !(item.platform === platform && item.externalId === externalId));
        render();
...
function render() {
    ...
    videoList.innerHTML = filtered
        ...
        const notes = [...content.notes]
             ...
             */
            const timestamp = content.platform === "youtube" && note.timestamp !== null
                ? `
                        <a
                          href="${escapeHtml(content.url)}&t=${Math.floor(note.timestamp)}s"
                          target="_blank"
...
>
            ...
            .join("");
        const duration = content.duration !== null
            ? `
              ...
              `
            : "";
        const platformLabel = content.platform === "instagram" ? "Instagram" : "YouTube";
        return `
            <article
              class="video-card"
              data-platform="${escapeHtml(content.platform)}"
...
async function updateNote(externalId, noteId, text) {
    ...
    if (!response?.success) {
        throw new Error(response?.error ?? "Errore durante la modifica della nota.");
    }
}
document.addEventListener("click", async (event) => {
    const target = event.target;
    const noteText = target.closest(".note-text");
    if (!noteText) {
        return;
    }
    /*
    ...
    const noteItem = noteText.closest(".note-item");
    if (!noteItem) {
        return;
    }
...
function startNoteEditing(noteItem, note, externalId) {
  ...
  `;
    ...
    const saveEditing = async () => {
        ...
        try {
            ...
            showToast(error instanceof Error ? error.message : "Errore durante la modifica.", "error");
        }
    };
...
async function sendDeleteMessage(message) {
...
function showConfirmModal(title, message) {
    return new Promise((resolve) => {
      ...
      `;
        ...
        const deleteButton = backdrop.querySelector(".confirm-modal-delete");
        let resolved = false;
        const close = (value) => {
            if (resolved) {
                return;

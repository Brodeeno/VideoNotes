...
let noteButton: HTMLButtonElement | null = null;
...
function openNoteDialog(info: VideoInfo): void {
  /*
  ...
  const dialog = backdrop.querySelector<HTMLDivElement>(
    ".youtube-notes-dialog",
  );

  const textarea = backdrop.querySelector<HTMLTextAreaElement>("textarea");

  const save = backdrop.querySelector<HTMLButtonElement>(
    ".youtube-notes-dialog-save",
  );

  const cancel = backdrop.querySelector<HTMLButtonElement>(
    ".youtube-notes-dialog-cancel",
  );

  const close = backdrop.querySelector<HTMLButtonElement>(
    ".youtube-notes-dialog-close",
  );
  ...
  save.addEventListener("click", async () => {
    ...
    try {
      ...
      if (errorMessage.includes("Extension context invalidated")) {
        showToast(
          "Extension was updated. Refresh the YouTube page and retry.",
          "error",
          5000,
        );
...
function createNoteButton(): void {
  ...
  if (existing) {
    noteButton = existing as HTMLButtonElement;

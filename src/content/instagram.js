...
function isProfilePicture(img) {
    ...
    return (alt.includes("profile picture") || alt.includes("immagine del profilo"));
}
function extractThumbnailUrl() {
    ...
    if (currentVideo) {
        let ancestor = currentVideo.parentElement;
        let depth = 0;
        let thumbnailImage = null;
        while (ancestor && depth < 6 && !thumbnailImage) {
            const candidates = [
                ...ancestor.querySelectorAll("img"),
            ];
 ...
 * ---------------------------------------------------------
 */
function openInstagramNoteDialog(info) {
    ...
    const textarea = dialog.querySelector(".instagram-notes-textarea");
    const close = () => {
        overlay.remove();
    };
    dialog
    ...
    dialog
        ...
        const save = dialog.querySelector(".instagram-notes-save");
        save.disabled = true;
 ...
 * ---------------------------------------------------------
 */
function createInstagramNoteButton() {
 ...
 */
function findBlockBeforeSiblingWithIcon(startElement, nextBlockIconLabels) {
    let node = startElement;
    while (node && node.parentElement) {
        const nextSibling = node.nextElementSibling;
        if (nextSibling) {
            const hasMatchingIcon = nextBlockIconLabels.some((label) => nextSibling.querySelector(`svg[aria-label="${label}"]`));
            if (hasMatchingIcon) {
                return node;
            }
        }
...
function insertNoteButton() {
    ...
    if (isPost) {
        ...
        const shareIcon = Array.from(document.querySelectorAll("svg[aria-label]")).find((el) => {
    ...
    if (isReel) {
         ...
         */
        const likeIcon = document.querySelector('svg[aria-label="Mi piace"], svg[aria-label="Non mi piace più"], svg[aria-label="Like"], svg[aria-label="Unlike"]');
        ...
        if (likeBlock) {
            const likeBlockElement = likeBlock;
            /*
             ...
             */
            const currentPosition = window.getComputedStyle(likeBlockElement).position;
...
function showToast(message, type = "success", duration = 3000) {
    ensureToastStyles();
    let container = document.querySelector(".instagram-notes-toast-container");
    if (!container) {
